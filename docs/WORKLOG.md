...
- Поиск existing issues стал обрабатываться детерминированно для явных фраз вида `дай список открытых issue`, `покажи закрытые issues`, `найди все issues`
- Служебные слова (`дай`, `список`, `open/closed`, `issue/issues`, `про`, `по`) вычищаются из поискового текста перед запросом к GitHub
- Если пользователь просит просто список по статусу без темы, в GitHub уходит пустой текстовый запрос и бот показывает список issues по фильтру состояния
- Для пустого поискового запроса бот теперь использует repo issues listing API и может показывать весь список по статусу, а не только первые 5 результатов search API
- Добавлены словоформы `всех/всем/всеми` для корректной обработки фраз типа `дай список всех issue`
